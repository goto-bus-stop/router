...
fn flatten_inputs_graphql_name(inputs: &[GraphQLSelection]) -> HashSet<String> {
    inputs
        .iter()
        .flat_map(|s| match s {
            GraphQLSelection::Field(f) => {
                if f.selection_set.is_empty() {
                    if f.name == "__typename" {
                        ...
                        HashSet::from([f.name.as_str().to_string()])
                    }
                } else {
                    flatten_inputs(&f.selection_set)
                        .iter()
                        .map(|child| format!("{}__{}", f.name, child))
                        .collect()
                }
            }
...
fn flatten_inputs(inputs: &[GraphQLSelection]) -> HashSet<String> {
    inputs
        .iter()
        .flat_map(|s| match s {
            GraphQLSelection::Field(f) => {
...
fn flatten_requires(requires: &[Selection]) -> HashSet<String> {
    requires
        .iter()
        .flat_map(|s| match s {
...
mod tests {
    ...
    fn test_finder_field_for_connector() {
        let schema = Schema::parse_and_validate(SCHEMA, "test_supergraph.graphql").unwrap();
